...
android-build:
    ...
    if [ ! -f "$$KEYSTORE" ]; then keytool -genkey -v -keystore "$$KEYSTORE" -alias androiddebugkey -storepass android -keypass android -keyalg RSA -keysize 2048 -validity 10000 -dname "CN=Android Debug,O=Android,C=US"; fi
    cd mobile && npx @tauri-apps/cli@^2 android init || true
    cd mobile && npm install
    NDK_DIR=$$(ls -1d "$${ANDROID_SDK_ROOT:-$$HOME/Android/Sdk}/ndk/"*/ 2>/dev/null | head -1)
    if [ -n "$$NDK_DIR" ]; then export TARGET_RANLIB="$$NDK_DIR/toolchains/llvm/prebuilt/linux-x86_64/bin/llvm-ranlib"; export CARGO_TARGET_AARCH64_LINUX_ANDROID_RANLIB="$$NDK_DIR/toolchains/llvm/prebuilt/linux-x86_64/bin/llvm-ranlib"; export PATH="$$NDK_DIR/toolchains/llvm/prebuilt/linux-x86_64/bin:$$PATH"; fi
    ...
    apksigner sign --ks "$$KEYSTORE" --ks-key-alias androiddebugkey --ks-pass pass:android --key-pass pass:android "$$ALIGNED"
    mv -f "$$ALIGNED" "mobile/yse-android-universal-release.apk"
    echo "APK ready: mobile/yse-android-universal-release.apk"
