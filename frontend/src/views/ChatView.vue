          ...
          >
            ...
            >
              <MessageBubble
                :message="msg"
                :alignment="
                  nameFromAddr(msg.from) === ownAddress ? 'self' : 'other'
                "
                :is-pending="isPending(msg)"
                :pending-status="
                  isPending(msg) ? (msg as any).__status : undefined
                "
                :pending-msg="msg"
...
const contacts = computed<Contact[]>(() => {
  ...
  const result = Array.from(map.values()).map((c) => {
    ...
    return {
      ...c,
      hasNew:
        selectedContact.value !== c.address && c.lastTime > ts && !c.lastIsSelf,
    };
  });
...
const hasUnreadContact = computed(() =>
  visibleContacts.value.some((c) => c.hasNew),
);
...
const conversation = computed(() => {
  ...
  const result: any[] = [];
  const pending = store.pendingMessages.filter(
    (p) => p.to === sel || p.from === sel,
  );
...
function onSwipeEnd(e: TouchEvent) {
  if (
    msgTouchIsSwipe &&
    e.changedTouches[0].clientX - msgTouchStartX > 100 &&
    isMobile.value &&
    selectedContact.value
  ) {
    selectedContact.value = "";
...
function isPending(msg: any): boolean {
  return !!(msg as any).__pending;
}
